...
import { downloadFileFromStorage, getThumbnailImageFirebaseFilePathFromRecipeId, getThumbnailImageLocalFilePathFromRecipeId, uploadFileToStorage } from "../utilities/FirebaseImageHandling.js";
...
router.post('/create', async (req, res, next) => {
    try {
        ...
        const localFilePath = getThumbnailImageLocalFilePathFromRecipeId(newRecipeRef.id)
        const cloudFilePath = getThumbnailImageFirebaseFilePathFromRecipeId(newRecipeRef.id)
...
router.get('/mine', async (req, res, next) => {
    try {
        const myRecipes = await db.collection('recipes').where('owner', '==', req.user.uid).get()
...
router.get('/specific', async (req, res, next) => {
    ...
    try {
        ...
        const thumbnailImageCloudPath = getThumbnailImageFirebaseFilePathFromRecipeId(recipeId)
        const recipeThumbnail = await downloadFileFromStorage(thumbnailImageCloudPath)
